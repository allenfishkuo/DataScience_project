# -*- coding: utf-8 -*-
"""
Created on Sat Dec  5 14:18:05 2020
...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=25):
        ...
        self.fc = nn.Linear(256*3 + 1, num_classes)
    ...
    def forward(self, input):
       # print("input:",input.shape)
        ...
        #print("y",y.size())
        #print("z",z)
        #print("converge_time",converge_time)
        out = torch.cat([x, y, z,converge_time], dim=1)
        #print(out.shape)
        #print(out.shape)
        out = out.squeeze()
        #print(out.shape)
...
def model_train(loader_train,loader_test):
    ...
    """
    msresnet = MSResNet(input_channel=1, layers=[1, 1, 1, 1], num_classes=25) #channel 設定
    msresnet = msresnet.cuda()
    criterion = nn.CrossEntropyLoss(size_average=False).cuda()
    optimizer = torch.optim.Adam(msresnet.parameters(), lr=0.0005)
    #optimizer = torch.optim.SGD(msresnet.parameters(), lr=0.01,momentum = 0.9)
    ...
    for epoch in range(EPOCH):
            ...
            if train_accuracy >= 90 and profit > big_profit :
                big_profit = profit
                torch.save(msresnet,"2015-2016NewNew_tp_withct.pkl")
...
def draw(train_loss, train_acc,winrate):
    ...
    df = pd.DataFrame({'loss': train_loss,
                   'acc': train_acc,
                   'winrate': winrate})
    df.to_csv("2015-2016NewNew_tp_withct.csv",index=False)

if __name__=='__main__':
    choose = 1
    if choose == 0 :
        train_data, train_label, test_data, test_label = new_dataloader_withct.read_data()
        loader_train,loader_test = DataTransfer(train_data, train_label, test_data, test_label)
        model_train(loader_train,loader_test)
    else :
    #model_train()
        test_withct.test_reward()
        #test_json.test_json()
        #pkl_to_pth.trans()