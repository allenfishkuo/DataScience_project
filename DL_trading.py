...
import test , test_withnews, backtest_new
import test_profit
#import test_cluster
...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=6):
        self.inplanes3 = 64
        self.inplanes5 = 64
        self.inplanes7 = 64
...
def model_train(loader_train,loader_test):
    ...
    """
    msresnet = MSResNet(input_channel=1, layers=[1, 1, 1, 1], num_classes= 6) #channel 設定
    ...
    for epoch in range(EPOCH):
            ...
            if train_accuracy >= 90 and profit > big_profit :
                big_profit = profit
                torch.save(msresnet,"2016-2016_6action.pkl")
...
def draw(train_loss, train_acc,winrate):
    ...
    df = pd.DataFrame({'loss': train_loss,
                   'acc': train_acc,
                   'winrate': winrate})
    df.to_csv("2016-2016_6action.csv",index=False)
